...
- **Дефолтные seat-боты:** сервер сам создаёт сетку из 10 мест и расставляет ботов по `room.context` (порядок мест `(x,y)`) — при **создании новой комнаты** в `join` (дубликат / spare) и при **первом join** в существующую; **STUDY:** `STUDY_HELPER` и `WORK_FOCUS_BUDDY`; **WORK:** `WORK_FOCUS_BUDDY`; **SPORT:** `SPORT_CHEERLEADER`; **leisure:** ботов нет. В лобби **`GET /api/rooms`** уже видно `seats[].occupiedBy` у пустой комнаты (после провижининга). Клиенту не нужно вызывать `POST .../seats/.../bot` при входе — идемпотентно по `botType` в комнате.
  ...
  - Когда выходит **последний человек**, дефолтные seat-боты **остаются** за столом; событий снятия ботов из-за `leave` нет.
