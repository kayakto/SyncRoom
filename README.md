...
- `topParicipants` — топ участников комнаты (до 5), каждый элемент: `{ "id": "...", "name": "...", "avatar_url": "..." }` (если аватара нет — `avatar_url: null`, клиент может показать заглушку по первой букве `name`).

**Структура комнаты** включает `backgroundPicture`, `seats[]` (с нормализованными координатами 0.0–1.0) и `topParicipants`.
